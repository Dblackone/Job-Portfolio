# Consulting Outreach Follow-up Batch — 6 August 2026

**Status:** Review only. Not sent.

**Purpose:** Follow up with the five companies contacted on 4 August 2026. Send only after checking the original thread, recipient, and current relevance. A reply or identifiable expression of interest is required before creating a lead in `leads.csv`.

## Sending checklist

- Open the original sent thread in Gmail.
- Confirm the recipient and company context still match.
- Use the follow-up below as the body of a reply, not as a new unrelated thread.
- Record the send date in `prospect-list.csv` after sending.
- Move the prospect to `leads.csv` only if the recipient replies or otherwise expresses interest.
- Do not claim a meeting, scope, fee, or value until it is confirmed.

## APD Project Management Limited

**Offer:** Construction project controls  
**Original subject:** Delivery reporting support for a live project

Hello APD Project Management Team,

Following up on my note about practical project-controls support for construction delivery.

If a current project would benefit from clearer programme, procurement, or reporting visibility, I can start with a focused review of the existing reporting rhythm and the immediate delivery risk. If relevant, please share the project phase and the area that currently needs the most attention.

Relevant project and consulting evidence: https://dblackone.github.io/Job-Portfolio/web-portfolio.html#engagements

Regards,  
Vollmann Akarakiri

## Amana Consortium Limited

**Offer:** BIM model and drawing review  
**Original subject:** Independent BIM review support for project information

Hello Amana Consortium Team,

Following up on my note about an independent review of Revit models and drawing packages.

If there is a live package approaching coordination, tender, or site use, I can review the current information against the project stage and return a focused list of coordination and documentation issues. If relevant, please share the project stage, disciplines included, and the review deadline.

Review scope: https://dblackone.github.io/Job-Portfolio/consulting/bim-model-review.html

Regards,  
Vollmann Akarakiri

## S and Bros Nigeria Limited

**Offer:** Construction project controls  
**Original subject:** Practical project-controls support for construction delivery

Hello S and Bros Team,

Following up on my note about strengthening the control rhythm across programme, procurement, cost, and reporting.

If one active project would benefit from a more structured delivery view, I can begin by reviewing the current reporting format and the decisions that need clearer visibility. If relevant, please share the project phase and the immediate reporting or coordination gap.

Relevant project and consulting evidence: https://dblackone.github.io/Job-Portfolio/web-portfolio.html#engagements

Regards,  
Vollmann Akarakiri

## AOS Project Management Limited

**Offer:** Construction project controls  
**Original subject:** Support for programme and project-decision visibility

Hello AOS Project Management Team,

Following up on my note about programme and project-decision visibility.

If a current development needs a clearer view of programme movement, decisions, or delivery risks, I can review the existing control rhythm and identify a practical starting point. If relevant, please share the project stage and the decision or reporting area that needs attention.

Relevant project and consulting evidence: https://dblackone.github.io/Job-Portfolio/web-portfolio.html#engagements

Regards,  
Vollmann Akarakiri

## ANU QUAD & CO

**Offer:** Design-to-site coordination  
**Original subject:** Design-to-site coordination support

Hello ANU QUAD & CO Team,

Following up on my note about moving design information into coordinated, buildable site decisions.

If a current project is experiencing gaps between drawings, consultants, and site execution, I can begin with a focused review of where coordination is breaking down and who owns the next decisions. If relevant, please share the project stage and the coordination issue currently affecting delivery.

Relevant BIM and project evidence: https://dblackone.github.io/Job-Portfolio/portfolio/bim.html

Regards,  
Vollmann Akarakiri

## Response checkpoint — 2026-08-05

Gmail review confirmed that follow-ups to all five contacts are present in Sent:

- APD Project Management Limited
- Amana Consortium Limited
- S and Bros Nigeria Limited
- AOS Project Management Limited
- ANU QUAD & CO

No reply from any of the five contacts was found in the mailbox at the time of
review. This is a response checkpoint, not evidence of rejection, lead status,
or income. Keep each contact outside `leads.csv` until an identifiable reply or
referral creates a real next step.
